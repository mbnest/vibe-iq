...
*Working notes, 2026-09-04. Output of a pre-planning conversation between Matt and Claude. The platform touches "rules" in two unrelated ways that are easy to conflate: the **governance decisions that run the pipeline**, and the **business rules that end up inside the product being built**. This doc separates them and places each. Draft, not a mandate. Companion to [`vibe-engineering-platform-notes.md`](vibe-engineering-platform-notes.md). Triggered by the question of whether a business-rule management system such as GoRules fits the architecture.*
