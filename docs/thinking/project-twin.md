...
*Working notes, 2026-09-04. Idea surfaced in a conversation between Matt and Claude, captured here as a build-ready concept for after the MVP — not scheduled, not started. Companion to [`architecture-napkin.md`](architecture-napkin.md) (where it eventually plugs in) and [`requirement-change-handling.md`](requirement-change-handling.md) (the change-management thread that motivated most of the design). Draft, not a mandate.*
...
- **Not a new decision-maker.** It does not classify changes or decide responses. It hands reasoning off to the impact/classification agent already described in [requirement-change-handling.md](requirement-change-handling.md) — same division of labor whether a change signal arrives via the twin, a ticket edit, or a stakeholder message.
...
3. **Blocking behavior while awaiting approval.** Whether only the affected subgraph blocks, or more — ties to the dependency-graph mechanism already described in [requirement-change-handling.md](requirement-change-handling.md).
...
7. **Friction budget.** Per [requirement-change-handling.md](requirement-change-handling.md)'s own warning, any confirm/notice step heavy enough will get routed around — the concrete UX for each notice type and the confirm step needs to stay near-zero-friction.
