...
*Working sketch, 2026-09-04. Major components of the platform on one page, with the MVP scope highlighted against the full system. Deliberately rough — it collapses detail that the other thinking docs carry. Companion to [`vibe-engineering-platform-notes.md`](vibe-engineering-platform-notes.md); the rigorous process view is in [`sdlc-business-process-flow.md`](sdlc-business-process-flow.md).*
...
- `Governance decision logic` and `Artifact store` exist in the MVP in a minimal form — pure functions for the first, git for the second — behind a clean interface, to be swapped for real components later ([`rules-and-policy.md`](rules-and-policy.md)).
