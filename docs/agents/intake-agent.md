...
**Role:** Intake & Requirements Clarification · [pipeline order](README.md) · [master diagram](../architecture/process-flow.md)
...
- Minimum-scaffold pre-check, routing back to Scoping when unmet
...
- **Receives from:** [Scoping Agent](scoping-agent.md) in the current master diagram. In the v0.5 front-of-flow model ([`front-of-flow.md`](../architecture/front-of-flow.md)), Intake runs **per story**, downstream of the [Definition Agent](definition-agent.md) and solution sign-off — it receives the frozen L2 product definition (and its feature/story slice) as a constraint boundary, not a raw scoped ask. The minimum intake scaffold pre-check and gap routing still apply; the Definition Agent reuses the same scaffold at the front of the flow.
- **Delivers to:** [Requirements Sign-off Gate](requirements-signoff-gate.md). Also escalates directly to the appropriate technical or compliance stakeholder mid-process when a gap is routed to them, rather than always resolving gaps through the original submitter first.
