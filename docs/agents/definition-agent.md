...
**Role:** Definition (L1→L2 product definition) · [pipeline order](README.md) · [master diagram](../architecture/process-flow.md)

Front of the flow — after [Scoping](scoping-agent.md) and the prioritization decision, before solution analysis. Part of the front-of-flow model ([`front-of-flow.md`](../architecture/front-of-flow.md)).

*New 2026-09-05. Draft — a working position to challenge, like every other agent doc. Phase decisions: [`../decisions/log.md`](../decisions/log.md#product-definition-phase-2026-09-05).*
...
- Business brief / intake request (L1), from [Scoping](scoping-agent.md), checked against the minimum intake scaffold ([Intake Agent](intake-agent.md))
- Current roadmap, OKRs, strategy context (where reachable)
- Existing product / capability inventory (the [Project Twin](../notes/project-twin.md), where it exists)
...
- **Sidecar metadata** — derivation links, version lineage, staleness flags (portable across storage backends — see [`product-definition-integration-options.md`](../notes/product-definition-integration-options.md)).
...
2. **Build the [Project Twin](../notes/project-twin.md)** — the current-state model of products and capabilities that exist today.
...
**Permanent constraint:** decision rights never transfer with data. Full data access makes the agent a better *advisor* for the portfolio / ambition / budget calls — never the decider. Consistent with [rules-and-policy.md](../policies/rules-and-policy.md) decision 10 (human gates stay human by design).
...
- **Receives from:** [Scoping Agent](scoping-agent.md), once the product function has made the prioritization decision.
- **Delivers to:** Solution analysis (front-of-flow) with the frozen, signed L2 product definition; then per-story [Intake](intake-agent.md) inherits the relevant slice as constraint boundary.
- **Change requests** after product-definition sign-off re-enter here (cheap path) until solution sign-off passes, after which they route through [requirement-change-handling](../policies/requirement-change-handling.md) (expensive path).

## Open sub-items (from [`product-definition-phase-open-questions.md`](../notes/product-definition-phase-open-questions.md))
