...
*Current design position (pre-planning, 2026-09-03). Draft, open to challenge. Covers the "beginning of the flow" — how a business need becomes a signed solution and a well-formed backlog, before per-story build begins. Draft, not a mandate; every position here is open to challenge. Companion to [`platform-notes.md`](../notes/platform-notes.md) and [`process-flow.md`](process-flow.md). Follow-up questions are collected at the end, deliberately unresolved — the current focus is the front of the flow, not merge/release.*
...
- **Provenance tag.** The existing marker on a requirement recording where the fact came from — `[stated]` / `[default]` / `[inferred]` / `[unresolved]` (see [`agents/intake-agent.md`](../agents/intake-agent.md)). Level tags are a sibling concept on a separate axis: provenance says *how solid* a requirement is, level says *how precise* it is.
...
- **Versioning + staleness propagation.** Change artifact #1 and the system flags exactly what in #2, #3, #4 is now stale. This is the mechanism the requirement-change work depends on — see [`requirement-change-handling.md`](../policies/requirement-change-handling.md).
...
- **13 phase decisions** are recorded in [`../decisions/log.md`](../decisions/log.md#product-definition-phase-2026-09-05) — artifact structure (structured records), freeze semantics (load-bearing forks block; metric targets must be filled), autonomy (publish safe, flag risky), the approval loop (product commits, then all affected stakeholders sign the mirror-back), and more.
- **Autonomy demarcation** — a per-function breakdown of what the agent can reason on now, what needs data access (strategy corpus, historical actuals, analytics, CRM, the [Project Twin](../notes/project-twin.md), the platform's own accumulated history), and what stays permanently human (the portfolio / ambition / budget decisions). Decision rights never transfer with data.
- **Integration possibilities** — [`product-definition-integration-options.md`](../notes/product-definition-integration-options.md) works through fitting alongside existing enterprise product tooling: separate the flow-contract from the implementation, define swappable ports, keep a sidecar metadata graph so the derivation chain survives wherever the content lives.

## Follow-up questions

Consolidated into [`../decisions/open-questions.md`](../decisions/open-questions.md) — see the "Front of the flow" and "Merge and release" sections there. In brief: the "level" vs. "altitude" term; whether feature-level solutioning always happens; whether solution analysis runs once or twice; what exactly the solution reviewer signs; the merge/release mechanics (parked); and how the level model interacts with a vertical's existing backlog.

Resolved since the first draft (2026-09-05): the L1→L2 work is an agent while prioritization stays human ([log.md](../decisions/log.md) D-17); the product definition is stored as structured requirement records (D-PD-01).

---

*Doc status: created 2026-09-03; Part 4 (Definition Agent) added 2026-09-05, when the fan-out, the solution gate, and the Definition Agent were folded into the [`process-flow.md`](process-flow.md) diagram. Not reviewed by anyone else. Open items now tracked in [`../decisions/open-questions.md`](../decisions/open-questions.md).*
