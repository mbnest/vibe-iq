...
*Current design position (pre-planning, 2026-09-04). Draft, open to challenge. Covers how the pipeline is orchestrated, where it runs for testing and demos, and which model tier fits which stage. Draft, not a mandate. Companion to [`platform-notes.md`](../notes/platform-notes.md) and [`rules-and-policy.md`](../policies/rules-and-policy.md). Model prices are Anthropic first-party rates as of the `claude-api` skill's 2026-06-24 cache — re-check before relying on a number.*
...
Established in decision #1 and [`rules-and-policy.md`](../policies/rules-and-policy.md): what step runs next, retries, timeouts, resume after a multi-day gate wait, and the fan-out from one initiative to N stories are all programmatic. The LLM reasons *inside* a step and produces an artifact; it never decides the path. Branches exist (gate or no gate, requirement-change re-entry point, catalog vs. novel path) but they are decided by the governance decision logic — rules and functions on explicit facts — not by a model.
...
- **CLI first.** The MVP already runs as a skill/prompt-loop on the Claude Agent SDK ([architecture-napkin.md](../notes/architecture-napkin.md)), which is CLI-native, and the MVP's single reviewer is technical. Building a web UI before a non-technical signer needs one solves a problem that doesn't exist yet — same discipline already applied to skipping an orchestrator for MVP.
...
- **The L2 product-definition form refines "no direct human edits" (2026-09-05).** The MVP's `spec.md` rule — reviewer talks to the agent, agent regenerates — assumed a technical reviewer on a terminal making a few changes. A product persona editing an L2 definition may make twenty edits in one sitting, and a chat cycle per edit is the wrong ergonomics. The resolution ([`agents/definition-agent.md`](../agents/definition-agent.md)): a **structured form is the primary editing surface**, the chatbot is a per-field escape hatch (and may write to fields directly, user confirms on submit), and the agent runs a **reconciliation pass on submit** — staleness propagation, downstream re-derivation, consistency check. The human edits fields; the agent still owns keeping the chain coherent. This preserves derivation *better* than a chat loop because the fields are typed and trace links explicit — it needs the artifact to be structured records underneath, which the Definition Agent's output is.
...
- This is already implied by the MVP path: "Intake agent → `spec.md` → requirements sign-off (**change-request loop, no direct human edits**) → Design agent" ([architecture-napkin.md](../notes/architecture-napkin.md)). The reviewer talks to that step's agent about the gap; the agent revises the artifact. It's naturally scoped to one step because each agent is `inputs -> artifact` in isolation.
- **This is not the [requirement-change-handling.md](../policies/requirement-change-handling.md) framework.** That governs changes discovered *after* a gate has already passed or work has been built. Catching a gap before sign-off is ordinary gate review — keep the line clear so a pre-approval tweak doesn't get routed through blast-radius/classification machinery it doesn't need.
...
- **On web it does need building:** a chat panel scoped to one artifact and one step's agent, next to the approve/reject controls. Structurally this is the same conversational-grounding pattern as the [project-twin](../notes/project-twin.md) — a chat interface anchored in project state — just narrowed from "the whole backlog" to "this one artifact." Worth treating as the same mechanism at two scopes rather than building it twice.
